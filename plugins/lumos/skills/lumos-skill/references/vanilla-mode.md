...
**Override the existing name — never add a parallel prefixed one.** When a project's real value differs from the foundation's placeholder default, redeclare the *same* custom property later in the cascade (section 2 already loads after the defaults, so this just works) — don't invent a project-specific variant name like `--_typography---font-size--acme-h1` sitting alongside the untouched `--_typography---font-size--h1`. A parallel namespace means the codebase now carries two font-size scales, two spacing scales, etc., and every component author has to know which one is "real" for this project. This applies even when a project's value happens to differ at every breakpoint (still override the same name) and even when it coincidentally *matches* the default at one endpoint but not the other (e.g. a project's main-text size sharing a mobile value with the foundation's `text-medium` but doubling it at desktop — that's still one token to redeclare, not a new one to add). Only give something a genuinely new name when the foundation has no corresponding slot at all (e.g. a project's own named spacing step like `space-80` with no foundation `space--80`, or a second display size the scale doesn't have) — and even then, no project-specific prefix: name it for what it is, the same way the foundation names its own tokens.

**Before overriding a name, check what else reads it — some tokens are load-bearing beyond their obvious use.** `--_typography---font-size--text-main` looks like it only affects text explicitly styled `u-text-style-main`, but the foundation's own default also points `--_text-style---font-size` (`page_wrap`'s base font-size) at `text-main`. A project whose real `text-main` is a large, prominent paragraph style (not a sane base reading size) will — by correctly overriding the name per the rule above — also inflate the whole page's inherited base font-size. `rem`-based CSS is unaffected (it always resolves against the root), but anything written in `em` inherits that font-size, including the `u-grid-above`/`u-grid-below` `@container` thresholds in this very file — a 32px base instead of 16px turns `62em` into ~1984px instead of ~992px, so a normal-width container reads as "narrow" and silently collapses to flex at desktop widths. If overriding a token changes a value something else structurally depends on, decouple the two explicitly (here: keep `text-main` at the project's real value, but re-point `--_text-style---font-size` at a fixed sane default like `1rem` instead of letting it ride on `text-main`) rather than accepting the side effect. Distilled from Stadtform (2026-07-06) — the grid silently ran in flex mode at every desktop width tested (1440–2560px) before this was traced back.
