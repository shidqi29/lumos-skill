...
4. **Customize by editing the placeholder value in-place — never leave the same variable defined twice with different values.** A variable must have exactly one definition; a second `:root` block that redefines a foundation token (e.g. a "PROJECT TOKENS" override with `--swatch--brand-500: #…` when the foundation already sets it) is a bug — only the later one wins and the dead placeholder confuses the next editor (and imports ambiguously to Webflow). For a project, either **overwrite the foundation's placeholder token** or add a **genuinely new** variable name.
...
- **When you re-author a library's own layout rules, treat every one of them as load-bearing until proven otherwise — including the ones that read as plain visual hygiene.** A slider root's `overflow: hidden` is the case that bites: it looks like "hide the other slides", but it also makes the element a **scroll container**, and a scroll container's min-content size is `0`. That zero is the thing quietly stopping N non-shrinking `width: 100%` slides from contributing their combined min-content to whatever sizes the root — a grid track, a flex line, the container itself. Lumos tells you to write `overflow: clip` instead of `hidden`, and `clip` is **not** a scroll container — so making that swap on a slider root removes the reset and closes a sizing cycle: the parent sizes from the slides, the slides size from the parent. Nothing errors; the browser just resolves it at its maximum length. Measured on a real build: one grid column at `33,554,406px` and a page `18,755,333px` tall, from deleting a single declaration. **Whenever you swap `hidden` → `clip` on an element whose children can't shrink, put `min-width: 0` back explicitly.** Same fix and same cause as the flex `min-width: 0` rule — it is the automatic minimum biting, not the width you wrote
- **Before you rename a library class to one of yours, check the target name isn't already reserved by the foundation.** A rename map is where `.is-*` names get handed out in bulk, and a few of them are not inert: a bare `.is-active` is part of the State Manager and flips `--_state---true` / `--_state---false` for the element **and everything inside it**. Give that name to a library's active-item class (a slider's `slideActiveClass`, a tab's active class) and every stateful descendant of the active item silently flips — buttons, links, anything reading `var(--_state---*)` — on that one item only, which is close to undebuggable. The "only `.is-active` for toggling state" rule governs state **you** manage; a class a library stamps at runtime is not that. Namespace it (`is-slide-active`), and check `.is-current` / `.is-open` too — they carry State Manager behavior wherever a `data-state` is in play
...
- **Images use a relative wrapper + an absolutely-filled `img`.** Put the `<img>` in a `_img_wrap` div that defines the box (`position: relative` + an `aspect-ratio`, or sized by the layout); the `<img>` fills it with `position: absolute; width: 100%; height: 100%` (object-fit cover is already the default). The skeleton loading color goes on the wrapper:
  ...
  ```
  **Never put the sizing on the `<img>` itself — an `aspect-ratio` there loses to the image's own `width`/`height` attributes.** A shot given `width: 66.97em; aspect-ratio: 361.66 / 241.107` directly on the `<img>` came out 362 wide (correct) and **1024 tall** — its intrinsic height, because the attribute-derived ratio won. The wrapper defines the box, the image fills it. This is the same `_img_wrap` rule as above, and it is easiest to skip inside a **scalable visual composition**, where the children read as bare artwork rather than as content images and the wrapper feels like an extra div.
...
- **In a multi-page project the class name must be unique across the SITE, not just within its component — put the page in the `{variation}` slot.** A Webflow class name is one global object for the whole site: there is exactly one `hero_wrap`, and it cannot mean something different on Home and About. So a section that every page has (hero, intro, banner) is named for the page it belongs to — `hero_home_wrap`, `hero_about_wrap`, `hero_pricing_wrap` — while a section only one page has keeps its plain name (`faq_wrap`, `story_wrap`). This is what the `{variation}` slot in `{type}_{variation}_{element}` is for. **The failure mode is invisible in vanilla**: each page loads its own `styles.css`, so three different `hero_wrap` rules coexist happily, every page renders correctly, and no sweep, render or pixel diff reports anything — the collision only exists once the pages are imported into one Webflow site, where the last import silently overwrites the others' styles with no way to separate them again. Audit it directly rather than by eye: parse each page stylesheet's selectors, intersect the class names, and require the intersection to be empty (a real build found 16 collisions, every one of them in the hero, because Figma names them all `hero_wrap`)
- **The 3-underscore cap is a forcing function, not a style preference — when a name hits it, promote the inner part to its own subcomponent instead of lengthening the name.** Adding the page scope above is what usually pushes an already-deep name over: `hero_visual_shadow_wrap` is fine until it becomes `hero_set_visual_shadow_wrap` (four). The fix is never to allow the fourth underscore; it is to recognise that the thing needing three levels of qualification IS a component — `setstack_wrap` / `setstack_img_wrap` / `setstack_shadow_wrap`. Hitting the cap is the signal that a subcomponent wants a name
...
- **Grid-first: every real multi-column layout is the grid — not `display: flex` with `flex: 1` children or fixed `width: Nrem` columns.** This is the whole point of Lumos. A row of equal columns → `u-grid-above` + `--_column-count---value: N` (drop the children's `flex: 1`/`min-width: 0`, add `width: 100%`); an asymmetric split → 12-col + `u-column-span-*` summing to 12; a fixed-width middle element (e.g. a portrait image flanked by text) → give it a **span**, not a rem width. Because `u-grid-above` collapses to a stacked column itself at the 62em threshold, delete the matching `@media { flex-flow: column }` overrides (and set any fixed card `height` to `auto` at that breakpoint). **Keep flex only for**: genuine visual compositions (overlapping/rotated/tilted image fans, tab bars), compact positioned groups that aren't a full-width band (a right-aligned stat cluster, a person-card), single-child centering wraps, form-field rows, and **label|value spec tables that must NOT collapse** (a `u-grid-above` collapse would split every pair — keep those as a non-collapsing 2-col). Swapping equal `flex:1` columns to `u-grid-above` is pixel-identical on desktop; verify and move on
...
- **A flex child that needs to fill its parent should use `flex: 1` (`.u-flex-grow`), not `height: 100%`, whenever the parent's height comes from `min-height`** (the 100svh full-bleed hero pattern is the common case: `hero_wrap { min-height: 100svh }`). `min-height` only sets a floor — the box can grow taller if its content needs more room — so it isn't a definite height a percentage can reliably resolve against; a child's `height: 100%` can end up sized to the original min-height instead of the box's actual rendered height. `flex: 1`/`flex-grow: 1` fills whatever space the flex container actually has, regardless of how that height was established (min-height, height, or content-driven growth)
- **An exact proportional split between items that have their own padding is grid `fr`, not `flex-grow` with `flex-basis: 0`.** With `box-sizing: border-box` a flex item can never be narrower than its own padding, so that floor is taken out of the pool **before** the grow ratio is applied — two padded panels asked for 615:412 come back 600:428, and `min-width: 0` does not help because the floor is padding, not content. `grid-template-columns: minmax(0, 615fr) minmax(0, 412fr)` divides the free space left after the gap and knows nothing about the items' padding. Reach for this whenever the proportion has to be exact rather than roughly right
- **An explicit `gap` on a container that is also `justify-content: space-between` is a FLOOR, not a spacing.** Space-between already distributes whatever is left over; the gap only takes effect when that leftover is *smaller* than it — at which point it pushes the container past the size the design gave it. In a column whose height the comp fixes, don't write a gap at all and let `justify-content` + the height do the work
- **A seam between two cells belongs to the cell that comes AFTER it — its leading edge IS the seam.** In a bento or any ruled grid, hang the row seam on a cell in the second ROW (its top edge) and the column seam on a cell in the second COLUMN (its left edge). Hung on the preceding cell instead, the line draws down the OUTSIDE of the grid, which looks plausible in isolation and wrong the moment you compare it to the design. The same rule applies one level down when the layout collapses: if the wrappers become `display: contents` on mobile, move the seams onto the cells, because a `display: contents` box paints no border and is not a containing block either
- **Prefer a border (or an edge-pinned child) owned by each cell over absolutely-positioned rules with computed offsets.** When a comp staggers its rules — two horizontals at different heights, two verticals at different x — the temptation is to place each one absolutely as a percentage of the section. Don't: those percentages only hold at the height the section happens to be, and they have to be recomputed every time the content changes. Give each column its own bottom/left edge instead and the stagger falls out for free, because the columns are genuinely different heights and each draws its own edge. Bleeding past the container is free too — the owning box already carries the negative margin, so the rule pinned to `left: 0` lands on the section edge with no arithmetic. Reach for absolute positioning only for a rule that is not a seam between two cells at all (a full-bleed line across the top of a section)
- **Paint order among siblings has to be stated once any of them is transformed.** A `transform` creates a stacking context, so a rotated/scaled sibling paints in a later phase than its un-transformed neighbours and lands on top of them **regardless of DOM order** — in an overlapping card fan that silently buries the un-rotated card's icon under its neighbour's corner. If the design stacks them in source order, give the un-transformed ones an explicit `z-index` to climb back. Scope it to the breakpoint that actually overlaps
- **To reserve an out-of-flow child's footprint, give the parent an `aspect-ratio` — don't fight the grid collapse.** When a design keeps two blocks overlapping at mobile (copy with a graphic layered over it), the readable build is: parent `position: relative` + `aspect-ratio` matching the graphic's footprint, child `position: absolute`. Because `min-height: auto` still applies, the parent grows if the copy ever outgrows the ratio — which is the stack-on-overflow fallback, for free. Do **not** re-force `display: grid` over the framework's `display: flex !important` collapse: answering an `!important` with another one leaves two rules insisting at each other for whoever reads it next
...
- `flex-grow` + `flex-basis: 0` to split padded items into an exact ratio — the padding floor is taken from the pool first and the ratio comes out wrong; use grid `minmax(0, Nfr)`
- An explicit `gap` on a `justify-content: space-between` container whose size the comp fixes — the gap becomes a floor and pushes the container past that size
- Assuming DOM order decides which of two overlapping siblings paints on top once one of them is transformed — a `transform` makes a stacking context and wins regardless
- Re-forcing `display: grid` over the framework's `display: flex !important` grid collapse — reserve the footprint with `aspect-ratio` on the parent and take the overlapping child out of flow instead
...
- Copying a library's layout CSS verbatim when you re-author it, or dropping one of its rules because it looks decorative — its `overflow: hidden` root doubles as the min-content reset that keeps its own slides from sizing their parent (swap to `clip` and you must add `min-width: 0`)
- Renaming a library's active/current/open class to a bare `.is-active` / `.is-current` / `.is-open` — those names carry State Manager behavior and will flip `--_state---*` for everything inside the active item
- `aspect-ratio` on an element with no explicit `width`/`height` alongside it — Safari won't compute it correctly
- `width` + `aspect-ratio` on the `<img>` itself instead of on its wrapper — the image's own width/height attributes win and it renders at its intrinsic height
- The same component class carrying different styles on two pages (`hero_wrap` on Home and on About) — one Webflow class name is global to the whole site; put the page in the `{variation}` slot
- Lengthening a name to a fourth underscore instead of promoting the inner part to its own subcomponent
- A grid seam hung on the cell BEFORE it — the line draws down the outside of the grid; the seam belongs to the cell after it
- Absolutely-positioned rules with percentage-of-section offsets where a border owned by each cell would do
